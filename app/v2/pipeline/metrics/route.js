...
export default Route.extend({
  ...
  beforeModel() {
    this.set('pipeline', this.modelFor('v2.pipeline').pipeline);
  },
  afterModel() {
  ...
  deactivate() {
    this.reinit();
    this.controllerFor('v2.pipeline.metrics').reinit();
  },
  async model({ jobId = this.jobId }) {
    const controller = this.controllerFor('v2.pipeline.metrics');
    ...
    const jobsMap = await this.shuttle
      .fetchJobs(pipelineId)
      .then(jobs =>
        jobs.reduce((map, j) => {
          const { id, name } = j;
