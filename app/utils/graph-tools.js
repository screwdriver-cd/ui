...
const decorateGraph = ({ inputGraph, builds, jobs, start }) => {
  ...
  const buildsAvailable =
    (Array.isArray(builds) || builds instanceof DS.PromiseArray || DS.PromiseManyArray) &&
    builds.length;
  const jobsAvailable =
    (Array.isArray(jobs) || jobs instanceof DS.PromiseArray || DS.PromiseManyArray) && jobs.length;
  const { edges } = graph;
