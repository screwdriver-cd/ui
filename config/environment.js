...
module.exports = environment => {
  ...
  // add default settings to allow in CSP settings
  ...
  // add loaded settings to allow in CSP settings
  const connectSrcs = ENV.contentSecurityPolicy['connect-src'];
  const frameSrcs = ENV.contentSecurityPolicy['frame-src'];

  if (!connectSrcs.includes(ENV.APP.SDAPI_HOSTNAME)) {
    connectSrcs.push(ENV.APP.SDAPI_HOSTNAME);
    connectSrcs.push(ENV.APP.SDSTORE_HOSTNAME);
  }

  if (!frameSrcs.includes(ENV.APP.SDAPI_HOSTNAME)) {
    frameSrcs.push(ENV.APP.SDAPI_HOSTNAME);
    frameSrcs.push(ENV.APP.SDSTORE_HOSTNAME);
  }

  // loads third party JavaScripts
  if (ENV.APP.FEEDBACK_SCRIPT && ENV.APP.FEEDBACK_HOSTNAME) {
    // loads JavaScript
    ENV.contentSecurityPolicy['script-src'].push(ENV.APP.FEEDBACK_SCRIPT);
    // loads iframe generated by above JavaScript
