...
module('Acceptance | pipelines/:id/settings/cache', hooks => {
  setupApplicationTest(hooks);

  const BASE_URL = `/v2/pipelines/${PIPELINE_ID}`;
  const TEST_URL = `${BASE_URL}/settings/cache`;

  test('visiting /v2/pipelines/:id/settings/cache requires being logged in', async assert => {
  ...
  test.skip('visiting /v2/pipelines/:id/settings/cache as guest', async assert => {
