...
module('Acceptance | pipelines/:id/settings/preferences', hooks => {
  setupApplicationTest(hooks);

  const BASE_URL = `/v2/pipelines/${PIPELINE_ID}`;
  const TEST_URL = `${BASE_URL}/settings/preferences`;

  test('visiting /v2/pipelines/:id/settings/preferences requires being logged in', async assert => {
  ...
  test.skip('visiting /v2/pipelines/:id/settings/preferences as guest', async assert => {
  ...
  test('visiting /v2/pipelines/:id/settings/preferences', async assert => {
    await visit(TEST_URL);

    assert.strictEqual(currentURL(), TEST_URL);
    assert.dom('#settings-nav-link').exists();
    assert.dom('#settings-nav-link').hasClass('active');
    assert.equal(getPageTitle(), 'foo/bar > Settings > Preferences');
  });
});
