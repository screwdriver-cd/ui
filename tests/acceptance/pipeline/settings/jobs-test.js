...
module('Acceptance | pipelines/:id/settings/jobs', hooks => {
  setupApplicationTest(hooks);

  const BASE_URL = `/v2/pipelines/${PIPELINE_ID}`;
  const TEST_URL = `${BASE_URL}/settings/jobs`;

  test('visiting /v2/pipelines/:id/settings/jobs requires being logged in', async assert => {
  ...
  test.skip('visiting /v2/pipelines/:id/settings/jobs as guest', async assert => {
