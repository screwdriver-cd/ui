...
module('Acceptance | pipelines/:id/settings', hooks => {
  setupApplicationTest(hooks);

  const BASE_URL = `/v2/pipelines/${PIPELINE_ID}`;
  const TEST_URL = `${BASE_URL}/settings`;

  test('visiting /v2/pipelines/:id/settings requires being logged in', async assert => {
  ...
  test.skip('visiting /v2/pipelines/:id/settings as guest', async assert => {
