...
module('Acceptance | pipelines/:id/secrets', hooks => {
  setupApplicationTest(hooks);

  const BASE_URL = `/v2/pipelines/${PIPELINE_ID}`;
  const TEST_URL = `${BASE_URL}/secrets`;

  test('visiting /v2/pipelines/:id/secrets requires being logged in', async assert => {
  ...
  test.skip('visiting /v2/pipelines/:id/secrets requires not being a guest', async assert => {
