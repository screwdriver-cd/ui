...
test('it renders', function (assert) {
  ...
  assert.equal($('th.appId').text().trim(), 'Name');
  assert.equal($('th.branch').text().trim(), 'Branch');
  assert.equal($('tr').length, 3);
  assert.equal($($('td.appId').get(0)).text().trim(), 'screwdriver-cd/screwdriver');
  assert.equal($($('td.appId').get(1)).text().trim(), 'screwdriver-cd/ui');
});
