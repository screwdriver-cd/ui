...
moduleForComponent('sd-build-log', 'Integration | Component | sd build log', {
  integration: true,
...
test('it renders closed', function (assert) {
  ...
  this.render(hbs`{{sd-build-log buildId=1 step=stepMock isOpen=false}}`);

  assert.equal(this.$().text().trim(), '');

  // Template block usage:
  this.render(hbs`
    {{#sd-build-log buildId=1 step=stepMock isOpen=false}}
      template block text
    {{/sd-build-log}}
  `);
...
test('it renders open when told to', function (assert) {
  ...
  this.render(hbs`{{sd-build-log buildId=1 step=stepMock isOpen=true}}`);
...
test('it starts loading when open changes', function (assert) {
  ...
  this.render(hbs`{{sd-build-log buildId=1 step=stepMock isOpen=open}}`);
  this.set('open', true);
