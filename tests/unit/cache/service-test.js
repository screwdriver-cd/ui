...
module('Unit | Service | cache', function(hooks) {
  ...
  test('it makes a call to delete pipeline cache successfully', function(assert) {
    server.delete('http://localhost:8080/v4/pipelines/1/caches?scope=pipelines&cacheId=1', () => [
      204
    ]);
    ...
    p.then(() => {
      ...
      assert.equal(
        request.url,
        'http://localhost:8080/v4/pipelines/1/caches?scope=pipelines&cacheId=1'
      );
    });
  });

  test('it makes a call to delete job cache successfully', function(assert) {
    server.delete('http://localhost:8080/v4/pipelines/1/caches?scope=jobs&cacheId=1', () => [204]);
    ...
    p.then(() => {
      ...
      assert.equal(request.status, '204');
      assert.equal(request.method, 'DELETE');
      assert.equal(request.url, 'http://localhost:8080/v4/pipelines/1/caches?scope=jobs&cacheId=1');
    });
  });
